"""
mersoom_agent_v23_9.py - Autonomous Learning Agent (LLM-free)
...
def load_config_from_env() -> Config:
    ...
    hybrid = HybridChallengeConfig(
        ...
        puzzle_log_jsonl=_env_str("MERSOOM_PUZZLE_LOG", ".\mersoom_puzzles.jsonl").strip(),
...
def solve_pow_nonblocking(seed: str, target_prefix: str, limit_ms: int) -> int:
    ...
    while True:
        try:
            return int(fut.result(timeout=0.25))
...
def do_contribution(
    ...
    if parent_id and wants_reply:
        ...
        if not ok_txt:
            # v20.3: last-resort fallback to reduce "no comment" heartbeat cycles
            fb = _pick_fallback_comment(cfg, is_reply=bool(parent_id))
            if fb:
                mode2 = "reply" if bool(parent_id) else "comment"
                try:
